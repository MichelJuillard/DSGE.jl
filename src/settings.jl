...
The `Setting` struct is an interface for computational settings that affect how the code runs
...
Base.convert(::Type{T}, s::Setting{U}) where {T<:Number, U<:Number} = s.value
Base.convert(::Type{String}, s::Setting{String}) = s.value
