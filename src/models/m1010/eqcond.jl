...
function eqcond(m::Model1010)
    ...
    Γ0[eq[:eq_euler], endo[:b_liq_t]]  = -1.
    Γ0[eq[:eq_euler], endo[:b_safe_t]] = -1.
    ...
    Γ0[eq[:eq_euler_f], endo[:b_liq_t]]    = -1.
    Γ0[eq[:eq_euler_f], endo[:b_safe_t]]   = -1.
    ...
    Γ0[eq[:eq_spread], endo[:b_liq_t]]   = (m[:σ_c]*(1 + m[:h]*exp(-m[:z_star])))/(1 - m[:h]*exp(-m[:z_star]))
    Γ0[eq[:eq_spread], endo[:b_safe_t]]  = (m[:σ_c]*(1 + m[:h]*exp(-m[:z_star])))/(1 - m[:h]*exp(-m[:z_star]))
    ...
    Γ0[eq[:eq_spread_f], endo[:b_liq_t]]   = (m[:σ_c]*(1 + m[:h]*exp(-m[:z_star])))/(1 - m[:h]*exp(-m[:z_star]))
    Γ0[eq[:eq_spread_f], endo[:b_safe_t]]  = (m[:σ_c]*(1 + m[:h]*exp(-m[:z_star])))/(1 - m[:h]*exp(-m[:z_star]))
    ...
    Γ1[eq[:eq_nevol], endo[:b_liq_t]]  = m[:ζ_nR]*((m[:σ_c]*(1+m[:h]*exp(-m[:z_star])))/(1-m[:h]*exp(-m[:z_star])))
    Γ1[eq[:eq_nevol], endo[:b_safe_t]] = m[:ζ_nR]*((m[:σ_c]*(1+m[:h]*exp(-m[:z_star])))/(1-m[:h]*exp(-m[:z_star])))
    ...
    Γ1[eq[:eq_nevol_f], endo[:b_liq_t]]  = m[:ζ_nR]*((m[:σ_c]*(1+m[:h]*exp(-m[:z_star])))/(1-m[:h]*exp(-m[:z_star])))
    Γ1[eq[:eq_nevol_f], endo[:b_safe_t]] = m[:ζ_nR]*((m[:σ_c]*(1+m[:h]*exp(-m[:z_star])))/(1-m[:h]*exp(-m[:z_star])))
