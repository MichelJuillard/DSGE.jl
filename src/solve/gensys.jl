...
function gensys(F::GeneralizedSchur, c, Ψ, Π, div)
    ...
    tmat = hcat(Matrix{Float64}(I, n - nunstab, n - nunstab), -(ueta * (deta \ veta') * veta1 * deta1 * adjoint(ueta1))')
    ...
    loose = G0I * vcat(etawt1 * (Matrix{Float64}(I, neta, neta) - veta*adjoint(veta)), zeros(nunstab, neta))
